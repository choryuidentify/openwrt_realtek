...
static void re865x_ring_free(struct re865x *re, struct re865x_ring *ring)
{
	if (ring->pkthdr_descs_cpu)
		dma_free_coherent(&re->pdev->dev, ring->num_descs * MBUF_DESC_SIZE,
...
static int re865x_ring_alloc(struct re865x *re, struct re865x_ring *ring, bool is_rx)
{
	int err;

	ring->pkthdr_descs_cpu = dma_alloc_coherent(&re->pdev->dev,
					ring->num_descs * sizeof (u32),
					&ring->pkthdr_descs_dma, GFP_KERNEL);
	if (!ring->pkthdr_descs_cpu) {
		err = -ENOMEM;
		goto err;
	}
	...
	if (is_rx) {
		ring->mbuf_descs_cpu = dma_alloc_coherent(&re->pdev->dev,
					ring->num_descs * sizeof (u32),
					&ring->mbuf_descs_dma, GFP_KERNEL);
		if (!ring->mbuf_descs_cpu) {
			err = -ENOMEM;
			goto err;
		}
	...
	ring->pkdhdrs_cpu = dma_alloc_coherent(&re->pdev->dev,
					ring->num_descs * PKDHDR_DESC_SIZE,
					&ring->pkdhdrs_dma, GFP_KERNEL);
	if (!ring->pkdhdrs_cpu) {
		err = -ENOMEM;
		goto err;
	}

	ring->mbufs_cpu = dma_alloc_coherent(&re->pdev->dev,
					ring->num_descs * MBUF_DESC_SIZE,
					&ring->mbufs_dma, GFP_KERNEL);
	if (!ring->mbufs_cpu) {
		err = -ENOMEM;
		goto err;
	}
...
err:
	...
	if (ring->pkdhdrs_cpu)
		dma_free_coherent(&re->pdev->dev, ring->num_descs * PKDHDR_DESC_SIZE,
				  ring->pkdhdrs_cpu, ring->pkdhdrs_dma);

	if (ring->mbuf_descs_cpu)
		dma_free_coherent(&re->pdev->dev, ring->num_descs * sizeof (u32),
				  ring->mbuf_descs_cpu, ring->mbuf_descs_dma);
			...
			int offset, void *(*alloc)(unsigned int size))
{
	...
	ring->mbufs[mbuf_idx]->rx.dma_addr = dma_map_single(&re->pdev->dev, data, re->rx_buf_size,
						DMA_FROM_DEVICE);
...
static void re865x_ring_rx_clean(struct re865x *re)
{
	...
	for (i = 0; i < ring->num_descs; i++) {
		...
		if (ring->mbufs[mbuf_idx]->rx.buf) {
			dma_unmap_single(&re->pdev->dev, ring->mbufs[mbuf_idx]->rx.dma_addr,
...
static int re865x_rings_init(struct re865x *re)
{
	...
	ret = re865x_ring_alloc(re, &re->tx_ring, false);
	if (ret)
		return ret;

	ret = re865x_ring_alloc(re, &re->tx_ring_dummy, false);
	if (ret)
		return ret;

	ret = re865x_ring_alloc(re, &re->rx_ring, true);
	if (ret)
		return ret;
...
{
	re865x_ring_free(re, &re->rx_ring);
	re865x_ring_free(re, &re->tx_ring);
	re865x_ring_free(re, &re->tx_ring_dummy);
}
					  ...
					  struct net_device *dev)
{
	...
	dma_addr = dma_map_single(&re->pdev->dev, skb->data, skb->len,
				  DMA_TO_DEVICE);
...
static int re865x_tx_packets(struct re865x *re)
{
	...
	for (; ring->bottom < ring->top; ring->bottom++) {
		...
		dma_unmap_single(&re->pdev->dev, ring->mbufs[i]->m_extbuf, ring->mbufs[i]->tx.skb->len, DMA_TO_DEVICE);
...
static void re865x_oom_timer_handler(struct timer_list *t)
{
	struct re865x *re = from_timer(re, t, oom_timer);
...
static int re865x_probe(struct platform_device *pdev)
{
	...
	re->sysctl = syscon_regmap_lookup_by_phandle(pdev->dev.of_node, "realtek,sysctl");
	if (IS_ERR(re->sysctl)) {
		dev_err(&pdev->dev, "failed to get sysctl registers\n");
		return PTR_ERR(re->sysctl);
	}
#endif

	/* disable interrupts, clear pending interrupts */
	re865x_reg_write(re, NIC_INTERRUPT_MASK_REG, 0);
	re865x_reg_rmw(re, NIC_CONTROL_REG, NIC_CONTROL_TX_CMD | NIC_CONTROL_RX_CMD, 0);
	...
	/* out-of-memory timer */
	timer_setup(&re->oom_timer, re865x_oom_timer_handler, 0);
...
static const struct of_device_id re865x_ids[] = {
	{ .compatible = "realtek,re865x-eth" },
	{},
};
...
static struct platform_driver re865x_driver = {
	...
	.driver = {
		.name	= DRV_NAME,
		.of_match_table = of_match_ptr(re865x_ids),
	}
};
