...
static inline void rtl865x_reg_rmw(struct rtl865x *rsw, unsigned reg, u32 clear, u32 set)
{
	...
	value = readl(rsw->swcore_base + reg);
	value &= ~clear;
	value |= set;
	writel(value, rsw->swcore_base + reg);
}
...
void rtl865x_port_init(struct rtl865x *rsw, struct device_node *np)
{
	...
	int phy_mode, size, id, shift;
	const __be32 *phy_fixed[8];
	u32 val, mask = 1;

	if (!_id || (be32_to_cpu(*_id) != 0 && be32_to_cpu(*_id) != 5)) {
		if (_id)
	...
	phy_fixed[id] = of_get_property(np, "realtek,fixed-link", &size);
	if (phy_fixed[id] && (size != (4 * sizeof(*phy_fixed[id])))) {
	...
	case PHY_INTERFACE_MODE_GMII:
		break;
	default:
		dev_err(rsw->parent, "port %d - invalid phy mode\n", id);
		return;
	}
	...
	if (id == 5)
		mask = 0;
	rtl865x_reg_rmw(rsw, RTL865X_SW_PORT_CONFIG_BASE, 3 << shift, mask << shift);
	...
	if (phy_fixed[id]) {
		...
		case SPEED_1000:
			...
			break;
		default:
			dev_err(rsw->parent, "invalid link speed: %d\n", speed);
			return;
		}

		val = val << 19; //ForceSpeed
...
#if defined(CONFIG_SOC_RTL8197D)
		{
			...
			if (reset) {
				...
				if (reset_gpio < 0) {
					...
					gpio_active_low = (flags & GPIOF_ACTIVE_LOW) == GPIOF_ACTIVE_LOW;
					err = gpio_request(reset_gpio, "re865x");
					if (err) {
						dev_err(rsw->parent, "gpio_request failed for %u, err=%d\n", reset_gpio, err);
					} else {
						gpio_direction_output(reset_gpio, gpio_active_low);
						gpio_set_value(reset_gpio, gpio_active_low);
						mdelay(1000);
						gpio_set_value(reset_gpio, !gpio_active_low);
						mdelay(1000);
...
static void rtl819x_fixup(struct rtl865x *rsw)
{
	int i;
#ifdef CONFIG_SOC_RTL8196C_REV_B
	u16 val;
#endif
	...
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x50c, 0x009400A0);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x510, 0x009400A0);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x514, 0x009400A0);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x518, 0x009400A0);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x51c, 0x009400A0);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x524, 0x009400A0);
#endif
...
#endif
#ifdef CONFIG_SOC_RTL8197D
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x904, 0x400b);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x908, 0xc0);
	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x910, 0x400b);

	rtl865x_reg_write(rsw, RTL865X_SW_GLOBAL_PORT_CTRL_BASE + 0x160, 0x28739ce7);
#endif
...
static void rtl819x_port_setup(struct rtl865x *rsw)
{
	int i;
	struct device_node *port;

#if defined(CONFIG_SOC_RTL8196C_REV_B) || defined(CONFIG_SOC_RTL8197D)
	rtl819x_fixup(rsw);
#endif
	...
	for_each_child_of_node(rsw->parent->of_node, port)
		if (of_device_is_compatible(port, "realtek,eth-port") &&
			   of_device_is_available(port))
			rtl865x_port_init(rsw, port);
}
...
static void rtl865x_reset(struct rtl865x *rsw)
{
	...
	regmap_update_bits(rsw->sysctl, RTL819X_SYSC_REG_CLK_MANAGE,
		CM_PROTECT | CM_ACTIVE_SWCORE,
		CM_PROTECT);
	mdelay(300);
...
#if defined(CONFIG_SOC_RTL8196C) || defined(CONFIG_SOC_RTL8197D)
